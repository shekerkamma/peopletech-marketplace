"""Reflector — reasoning half of the self-improving Stop hook.

Gathers the session's working-tree diff plus current CLAUDE.md of every
touched area, asks Claude (headless `claude -p`) to judge whether conventions
still hold, and writes the proposal to `.claude/claude-md-review.md`.

Runs in background (spawned by propose_claude_md.py) or directly for testing:
    python .claude/hooks/reflect_claude_md.py

Safety: recursion guard via PEOPLETECH_AILAYER_REFLECT_LOCK env var.
Fallback: if `claude` CLI missing, writes deterministic "re-check" note.
"""
...
This is a pre-sales deck workspace for Hyundai AI plant operations. The key \
invariant is: customer-facing materials must never mention internal tools \
(graphify, knowledge graphs, Obsidian, etc.).
...
def _run_claude(prompt: str, root: Path) -> str | None:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, encoding="utf-8",
            errors="replace", timeout=_CLAUDE_TIMEOUT, env=env,
        )
    except (OSError, subprocess.SubprocessError):
        return None
